// EFF Short Wordlist 1 (~1296 words, ~10.3 bits/word)
...
/**
 * Generate a diceware-style passphrase using the EFF short wordlist.
 * ~10.3 bits of entropy per word.
 */
export function generateDicewarePassphrase(wordCount = 7): string {
  const words: string[] = [];
...
/**
 * Generate a random passphrase from a mixed charset.
 * ~6.1 bits of entropy per character (70 chars in charset).
 */
export function generateRandomPassphrase(length = 20): string {
  const chars: string[] = [];
  for (let i = 0; i < length; i++) {
    chars.push(RANDOM_CHARSET[secureRandomIndex(RANDOM_CHARSET.length)]);
  }
  return chars.join("");
}
